...
3. GraphQL Schema editor displays *Type* and *Type Weight* information. The weighting factor is used when calculating the type cost for a request to the GraphQL API. For example, a field that requires extensive CPU or memory use on the server to retrieve its value would be given a higher cost. Click **View** in the *Warning* message.

![](images/apic-edit-3.png)

4. In addition to the warning details, the Warning window gives an option to fix the warning by applying the limits. Click **Apply all** (1), then click **Apply** (2).

![](images/apic-edit-4.png)

5. After applying the recommendation the warning message is removed. Click **Save** to save the API definition .
...
9. Enter **Authorization** as the new header allowed (1) and click **Save** button (2).
...
12. The Test feature also includes a GraphQL Editor. Enter the following JSON in the GraphQL Editor. You may use *Prettify* option to improve the visualization of the JSON.
```
{
  customers {
    email
...
14. Everytime that you invoke your API you need to pass your API Key as Authorization header. So let's include it in our test. On the bottom of the Parameters list, include a new parameter with the following value:
...
18. On the **Otherwise** ramification (1),  you will add a node before the *graphql-invoke* node. On the *Palette* view, scroll down to see the *Policies* section (2). Drag and drop the **Set Variable** (3) node between the "*ratelimit expected*" node and "*graphql-invoke*" node (4).
...
22. Let's test again our API, now without pass the Authorization token. Open the **Test** tab (1), and click the **Execute Query** button (2). (Note: if you want, you can check in the Parameters tab that there isn't an Authorization header parameter).

![](images/apic-edit-22.png)

23. Great! Now you GraphQL Proxy is running and available to be used. You are welcome to change the GraphQL query to test more scenarios. Next section you will learn how to create a product to apply GraphQL RateLimits. On breadcrumbs, return to **Develop** page.
...
5. The *Default Plan* contains traditional rate limiting options, but in next steps, you will be able to list GraphQL Rate Limits. By now, just click **Next**.
...
11. On the left menu, click **Manage** icon to manage your Sandbox catalog.
...
3.Great, here you have your **Portal URL**. Copy it, and open it in a new browser tab.
  ...
  'Note':  if you don't have a portal URL in your environment, go ahead and create a new portal. It will demand few minutes to have the portal ready.

![](images/apic-portal-3.png)

4. Click **Create account** to create a new developer account.

![](images/apic-portal-4.png)

5. Complete the registration form, go ahead and use any personal email.  Take note of the username (not email) and password. You will need them later to log in Developer Portal.
