...
| **Demo intro slides** | Download the Introduction and Overview slides <a href="./files/AsperaTechSellers-Presentation.pptx" target="_blank" rel="noreferrer">here</a>. |
...
2.6 Now, back to your email, you should have an email from IBM Aspera on Cloud. Open this email and click on **Access trial**.
...
3.1 First time you access the Aspera on Cloud, you should see the **Welcome** dialog, click **Enable high speed transfer**.

![](images/3-1-update-2024-1.png)

3.2 Now, you will see a page, asking you to install the IBM Aspera Connect transfer client. You need to install both the Aspera Connect browser plug-in and the Aspera Connect client. Let's do it! Click **Install extension**.  

![](images/connect-1.png)

3.3 Go ahead, and follow the necessary steps to install the extension in your browser.

![](images/connect-2.png)

3.4 When done, click **Install Connect** to open the <a href="https://www.ibm.com/aspera/connect/" target="_blank">IBM Aspera Connect</a> page. 

![](images/connect-3.png)

3.5 Go ahead and follow the necessary steps to install IBM Aspera Connect in your OS. 

![](images/connect-4.png)

3.6 When done, return back to the Aspera on Cloud page. Now, you should be on the Default Workspace area on Files app. Before explore the Files app, let’s open the admin page. Open the **App switcher** menu (A), select **Admin** (B).

![](images/3-6-update-2024-1.png)
...
4.1 Open the **Workspaces** page (A). Then click on **Default Workspace** (B) to edit it.
...
In this section and next, you will configure two Object Storage services in different cloud providers to use in the demo to show file transfer between different cloud providers. Because of configuration simplicity, in this demo, you will use IBM Cloud and Microsoft Azure. Microsoft enables you to use object storage service as part of free tier / trial account. The same is true with Google Cloud Platform and AWS. You are welcome to use any Object Storage service. IBM Cloud Object Storage, you need to request a TechZone COS service. Let's do it!

6.1 To request a TechZone Cloud Object Storage service, go <a href="https://techzone.ibm.com/my/reservations/create/66e30515c428e2fc24fa106f" target="_blank">here</a> and complete the form with the follow details.

| Field  | Value |
| ------------- | ------------- |
| Purpose  | If you are doing the L3 course, select **Education**. <br/> If you are a demo, select:**Demo** and inform the Sales opportunity.  |
| Purpose Description  | If you are doing the L3 course, enter: **Aspera L3 Course**. <br/> If you are delivering a Demo, enter the Demo details.  |
| Preferred Geography  | You are welcome to select the preferred Geography |

![](images/tz-cos-01.png)

6.2 **Agree** with the TechZone Terms and Conditions (A) and click **Submit** (B).
![](images/tz-cos-02.png)

6.3 You will receive an email confirming the instance is provisioning.
![](images/tz-cos-03.png)

6.4 You will receive a second email once the environment is provisioned. You should expect this to take between 5 and 20 minutes. Now the status update changed to **Ready**.  The COS service is created in a specific TechZone (TZ) IBM Cloud account, you can check the account name in this email (A). If you have not done so (in a previous reservation) you will need to click on the
notifications page link (B) to go and accept the invitation.

![](images/tz-cos-05.png)

6.5 Clicking on notifications page brings you to a login page where you need to provide your IBMid (and may have to log in with your credentials if you have
not already done so). Then you will see the Action required item. Click on the **item** (A). Then click **Join now**.

![](images/tz-cos-06.png)

6.6 You will now be sent back to the IBM Cloud login page. Use your IBMid to log in. You are asked to join the TechZone account. **Accept the Terms and Conditions** (A), then click **Join Account** (B).

![](images/tz-cos-07.png)

6.7 When you log in, ensure you are using the right account. Assuming that you have accepted the invitation, you should have an itz-watsonx -n account. Go to the menu bar and click on the icon to slide open the list of accounts available to you, and ensure that you select the account that was assigned to you from your current TechZone reservation. You can always check your reservation email to find the account name.

![](images/tz-cos-08.png)

6.8 On IBM Dashboard, open your **Resources** list.

![](images/tz-cos-09.png)

6.9 On *Resources* page, open **Storage** section and select your **Storage service**.

6.10 On your *Storage Instance* page, click **Create bucket**.

![](images/6-6-update-2024-1.png)

6.11 Select **Quickly get started** to use a predefined bucket.

![](images/6-7-update-2024-1.png)

6.12 Your IBM Cloud storage must be in a region that is supported by the Aspera on Cloud transfer service. To view the supported regions, check the <a href="https://ats.aspera.io/pub/v1/servers/softlayer" target="_blank">list</a> here.  Check that you are using **us-south** (A) and click **Next** (B).

![](images/icos-8.png)

6.13 **Drag and drop a 50MB file** (A) and click **Next** (B).

![](images/icos-9.png)

6.14 On *Test your bucket* page, click **View bucket configuration** button.

![](images/icos-10.png)

6.15 On the Bucket configuration page, take notes of your Bucket name (A), you will need that during the demo to integrate your ICOS bucket with Aspera on Cloud. On the Breadcrumbs, click on your new **Storage instance** link (B).

![](images/6-11-update-2024-1.png)

6.16 Open the **Service credentials** tab.

![](images/6-12-update-2024-1.png)

6.17 You need to have HMAC credentials for the IBM Cloud bucket. Let's create one! Click **New credential**.

![](images/icos-12.png)

6.18 Enter **Service credentials-HMAC** as Credential Name (A). Select **Writer** as Role (B). **Turn On** the *Include HMAC Credential* (C). And click **Add** (D).

![](images/6-14-update-2024-1.png)

6.19 Click on **Service credentials-HMAC** credential (A) and take note of the **access_key_id** and **secret_access_key** (B). You will need both during the demo.
...
7.4 On Storage accounts page, click on **+ Create**.

![](images/7-4-update-2024-1.png)
...
Then click on **Advanced** tab (C).

![](images/7-6-update-2024-1.png)

7.7 On the *Advanced* page, click on **Allow enabling anonymous access on indiviual containers** (A), and click **Review** (B).

![](images/7-7-update-2024-1.png)

7.8 Wait few seconds for the review process is ready. When completed, click **Create**.

![](images/azure-6.png)

7.9 You need to wait few minutes to deploy your storage account. When **your deployment is complete** (A), click **Go to resource** (B).

![](images/azure-7.png)

7.10 Now you need to create your Blob container. On *Data storage* section, click on **Containers**.

![](images/azure-8.png)

7.11 To create a new container, click on **+ Container**.
...
7.12 On the Containers view, click on your new **Blob container** to open it.

![](images/7-12-update-2024-1.png)

7.13 On Blob container, click **Upload** (A). Select a **10 Mb file** (B) and click **Upload** button (C).

![](images/azure-11.png)

7.14 Let's go back to the storage page. On the *Breadcrumbs*, click on your **Storage name**.

![](images/azure-12.png)

7.15 Click on **Access keys**.

![](images/azure-13.png)

7.16 On **key1** field, click **Show** button (A) and click **Copy** button (B) to copy the *Key* field. You will need that later in the demo to attach the Azure Blob Storage on Aspera on Cloud.
