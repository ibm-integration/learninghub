...
<InlineNotification kind="warning">There is a known issue with App Connect on AWS with a specific step of this demo. The IBM Development team is working to fix it as soon as possible. Because of it, there are some workaround steps to enable you to complete the demo. As soon as the issues are fixed, the workaround steps will be removed.</InlineNotification>
...
<InlineNotification kind="warning">There is a known issue with the Salesforce Retrieve contacts node when added by the AI-generated flow feature on App Connect on AWS. The IBM Development team is working to fix it as soon as possible. Because of it, the steps below are a workaround to manualy add another Salesforce Retrieve Contact node. As soon as the bug is fixed, steps 2.1.5 to 2.1.7 will be removed.</InlineNotification>

<br/>

**Action** &nbsp; 2.1.5 (WORKAROUND)

Click on the **Salesforce Retrieve contacts** node (A). And click to **Delete** it (B).

  ![](images/workaround-sfnode-1.png)

<hr/><br/>

**Action** &nbsp; 2.1.6 (WORKAROUND)

Click the **Plus** icon (A). Search for the **Salesforce** connector (B).

  ![](images/workaround-sfnode-2.png)

<hr/><br/>

**Action** &nbsp; 2.1.7 (WORKAROUND)

From your pre-configured Salesforce connector, select **Contacts** (A) > **Retrieve contacts** (B).

  ![](images/workaround-sfnode-3.png)
...
Click on the **Assistant name** field and select the **For each: Salesforce Contact/Contact > Full Name**.
...
Log in with your **Salesforce Dev account**. Open the **App Launcher** menu (A). Search for **Contacts** (B). And select the **Contacts** items (C).
