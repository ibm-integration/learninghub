...
| **Action** &nbsp; 2.3.3 | Confirm the instances has been deployed successfully before moving to the next step running the following command. ``` oc get pods -n cp4i | grep nginx ``` <br/><br/> You should get a response like this: <br/><br/>![](images/uniform-cluster-2-3-3.png)<br/>|
| **Action** &nbsp; 2.3.2 | Run the command below to create service for NGINX to be used by App. ``` oc apply -f resources/04c-nginx-service.yaml ``` |
