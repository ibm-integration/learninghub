...
| **Action** &nbsp; 2.2.3 | Run the command below.``` oc get queuemanager -n cp4i ``` <br/><br/> `Note` this will take few minutes, but at the end you should get a response like this: <br/><br/> |
...
| **Action** &nbsp; 3.2.3 | Select **Browse** and follow the dialogs to select the **jmsproducer-jgr-demo.jar** file (check the demo preparation document about how to get the jar file).  |
...
| **Action** &nbsp; 3.3.4 | Take your time reviewing the log, and at the bottom you will find a message saying “Sending…” confirming the application was able to connect to a queue manager and it is sending messages.  When done, **collapse** the log window. |
...
| **Action** &nbsp; 4.2.1 | Open the **Terminal** tab (1) and click **Expand** (2) |
...
| **Action** &nbsp; 4.3.1 | Right click the **Topology** menu and select **Open Link in New Tab** to keep a window open for each queue manager since you may need to go back and forth.  |
...
| **Action** &nbsp; 4.4.3 | Show the **connections** available. Once you are over you can **Collapse** the terminal again.  <br/><br/> `Note`: In your case the result could potentially be the opposite since we have no affinity defined and the application will connect to any queue manager, but since we only have one instance there will only be a connection to one queue manager at a time. |
...
| **Action** &nbsp; 5.1.1 | Right click on the **RedHat OpenShift logo** (1) and select **Open Link in New Tab** (2) to keep a window open.  |
| **Action** &nbsp; 5.1.2 | Change to **Administrator** perspective. |
| **Action** &nbsp; 5.1.3 | Open the **Workloads** menu (A) and select **Deployments** (B). |
| **Action** &nbsp; 5.1.4 | Click on the **jmsproducer** deployment. |
...
| **Action** &nbsp; 5.2.1 | Back to the **MQ1 browser tab**, enter the command below again and press **enter**. ``` echo 'dis conn(*)' all | runmqsc | grep -i my ``` |
...
| **Action** &nbsp; 6.1.1 | On the left navigator, open the **Operators** (A) > **Installed Operators** (B) menu.  |
...
| :--- | :--- |
| **Narration** | If we try to navigate back to the active pod for queue manager 2, we will get a “terminal connection has closed" error message since the queue manager and therefore its pods have been deleted already. <br/><br/>However, if we navigate to the active pod for queue manager one and submit the command to check the number of active connection, we will see all the connections are directed to the active queue manager assuring the client application can continue sending messages.  |
| **Action** &nbsp; 6.2.1 | Back to the **MQ2 browser tab**, show the error message: “**The terminal connection has closed**”. |
| **Action** &nbsp; 6.2.2 | Back to the **MQ1 browser tab**, enter the command below again and press **enter**. <br/><br/> ``` echo 'dis conn(*)' all | runmqsc | grep -i my ```  |
...
| **Action** &nbsp; 6.3.2 | Run the command below.<br/><br/> ``` oc get queuemanager -n cp4i ``` <br/><br/>`Note` this will take few minutes, but at the end you should get a response like this: |
...
| **Action** &nbsp; 6.4.1 | Back to the **MQ1 browser tab**, enter the command below again and press **enter**. <br/><br/> ``` echo 'dis conn(*)' all | runmqsc | grep -i my ``` |
| **Action** &nbsp; 6.4.2 | Show that you have **multiple connections** in **MQ1**. |
| **Action** &nbsp; 6.4.3 | Back to the **MQ2 browser tab**, enter the command below again and press enter.<br/><br/>``` echo 'dis conn(*)' all | runmqsc | grep -i my ``` |
