...
<inline-notification text="There is known issue in the current version of APIC on AWS to test GraphQL API. The Development team is working to fix it as soon as possible. The steps 7.4.2 to 7.4.7 are the steps to follow when the issues is fixed, by now there is an workaround available on step 7.4.8 to use external tool to test the API. As soon as the issue is fixed, the workaround step (7.4.8) will be removed!"></inline-notification>

<ToastNotification aria-label="closes notification"  kind="error" role="status" statusIconDescription="notification" subtitle="Subtitle text goes here"  title="Notification title"/>
