...
<InlineNotification kind="warning">There is a known issue editing the gateway policies in the new assembly editor on APIC on AWS. The IBM Development team is working to fix it as soon as possible. Because of it, the steps below are a workaround to set a flag to use the old editor. As soon as the bug is fixed, steps 6.1.4 to 6.1.6 will be removed.</InlineNotification>
...
On the *Feature flags* page, scroll down to see the last flag **Enable legacy assembly editor for gateway policies** and toggle the button from offline to **online**.
...
Click **View** on the *Warning* message notification box.
...
<InlineNotification kind="warning">There is a known issue with the GraphQL test tool on APIC on AWS. The IBM Development team is working to fix it as soon as possible. Because of it, please skip steps 7.4.3 - 7.4.8. Step 7.4.9 is a workaround to test your GraphQL Proxy API using an external tool. As soon as the issue is fixed, the step 7.4.9 will be removed.</InlineNotification>
...
Enter the following GraphQL query in the GraphQL Editor. You may use the **Prettify** option to improve the visualization of the GraphQL query.
...
Now you need to include the Authorization header. Open the **Parameters** tab.
...
On the Trace section, you should see a HTTP Status: 200 (1) and in the Response data you have the data returned from your GraphQL API (2).
...
<InlineNotification kind="warning">There is a known issue with the GraphQL test tool on APIC on AWS. The IBM Development team is working to fix it as soon as possible. Because of it, please skip steps 7.6.1 - 7.6.2. Step 7.6.3 is a workaround to test your GraphQL Proxy API using an external tool. As soon as the issue is fixed, the step 7.6.3 will be removed.</InlineNotification>
...
Show the result. You are welcome to change the GraphQL query to test more scenarios. You can also use the **Example** button to help start your creativity. On the breadcrumbs, return to the **Develop** page.
