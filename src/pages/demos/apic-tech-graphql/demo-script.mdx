...
<InlineNotification>There is a known issue with GraphQL test tool in APIC on AWS. The Development's team is working to fix it as soon as possible. Because of it, please skip steps 7.4.2 to 7.4.7. The step 7.4.8 is a workaround to test your GraphQL Proxy API using an external tool. As soon as the bug is fixed, the step 7.4.8 will be removed.</InlineNotification>
