...
To get the details about Focus’ account, first they need to navigate to the StepZen Dashboard. In the Account page, they will find their account name, and the two keys they need for API authentication.
...
To run StepZen in the cloud, the developers can connect the CLI to their account to get a private and secure endpoint or use StepZen with a public account. After installing the CLI, they are able to connect it to their account using the login CLI command.
...
Using StepZen, Focus’ developers can create a GraphQL API for backends with REST interfaces in minutes.
...
**Narration**
The StepZen dashboard provides a playground for testing the GraphQL API. In the Dashboard, Focus’ developers can play with different queries, for example, they can try the orders query. Another possibility is to run a customer query, with some custom fields. Remember, with this GraphQL API, they are consuming data from two different REST APIs. And if they want, they can do that with APIs from different types of data sources as well.
...
At this stage, the Focus’ tem decided to create a GraphQL API with a SQL database backend.
...
<InlineNotification>There is a known issue with GraphQL test tool on APIC on AWS. The Development's team is working to fix it as soon as possible. Because of it, please skip steps 7.4.2 to 7.4.7. The step 7.4.8 is a workaround to test your GraphQL Proxy API using an external tool. As soon as the bug is fixed, the step 7.4.8 will be removed.</InlineNotification>

<br/>

**Action** &nbsp; 7.4.2 (This step is currently not working, see the notification above)
...
**Action** &nbsp; 7.4.3 (This step is currently not working, see the notification on step 7.4.2)
...
**Action** &nbsp; 7.4.4 (This step is currently not working, see the notification on step 7.4.2)
...
**Action** &nbsp; 7.4.5 (This step is currently not working, see the notification on step 7.4.2)
...
**Action** &nbsp; 7.4.6 (This step is currently not working, see the notification on step 7.4.2)
...
**Action** &nbsp; 7.4.7 (This step is currently not working, see the notification on step 7.4.2)
