...
<AnchorLinks>
  ...
  <AnchorLink>9 - Configure Message Queue (IBM MQ) to Authorize and Accept Data</AnchorLink>
...
### 9 - Configure Message Queue (IBM MQ) to Authorize and Accept Data
...
9.1 Let’s configure the MQ security. Open a browser and go to this <a href="https://github.com/ibm-integration/learninghub/tree/main/static/mq-ace-files" target="_blank">GitHub page</a>.

9.2 Download both ZIP files (mqconfig.zip and ace-mq.zip).
![](images/mq-authorize-9-2.png)

9.3 Open a terminal window and enter unzip mqconf.zip. Unzip creates a directory mqconf.zip
```
unzip mqconf.zip
```

Then access the unzipped directory
```
cd mqconf
```

`Note`- Do not unzip ace-mq.zip (this is a App Connect Interchange Project).

9.4 Now, let's get the mq pods available on OpenShift server. First, let's use the **tools** project.
```
oc project tools
```

9.5 Now, let's find the check the mq pods. Use the command below.
```
oc get pods | grep mq
```

9.6 Take notes of the MQ pods, you will need this information later.
![](images/mq-authorize-9-6.png)

9.7 Execute the shell script to run MQ configuration. Execute the command below by replacing the [your_mq_pod_name].
```
./loadmq.sh [name of your mq pod]
```

![](images/mq-authorize-9-7.png)

The MQ configuration was updated on your QMGRDEMO queue manager.
...
10.2 Provide the requested details and **confirm your privacy settings and acceptance of the license**. Then select the appropriate download option (for example, ACE-WIN64-DEVELOPER.exe 1.26 GB), and click **Download**.

10.3 Install **IBM App Connect Enterprise for Developers** on your chosen platform. More details available on this <a href="https://www.ibm.com/docs/en/app-connect/12.0?topic=enterprise-download-ace-developer-edition-get-started" target="_blank">page</a>.

10.4 Start the **IBM App Connect Enterprise Toolkit** on your chosen platform.
